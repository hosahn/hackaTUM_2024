        ...
        for (let i = 0; i < length; i++) {
            ...
            try {
                const api_call = await axios.post(
                    ...
                    {
                        "messages": [{
                            "role": "user", "content": `Please get me a short summary (max. 2 sentences) of this article:${text} Don't answer with sure! or something like that. Please get straight to the point. And all of your result should be in English`
                        }],
                    },
                );
                const responseMessage = api_call.data.choices[0].message.content;
